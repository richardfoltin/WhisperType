...
Generated by spec-hub codemap — do not edit.
Coverage is total: every directory appears, as its own `##` section
or inside a parent's `dir/ (N files)` line; the token budget only
sets the level of detail. Detailed entries list a file's exported
symbols, most-used first; `imported by N` = N other files import
that file.
...
- jobs.py — imported by 3 files:
  ...
  - `@dataclass class ModelSwitch:`
  - `@dataclass class EngineSwitch:`
  - `@dataclass class ModelUnload:`
  - `@dataclass class TranscriptionJob:`
  - `@dataclass class BenchmarkResult:`
  - `@dataclass class BenchmarkJob:`
  - `class JobQueue:`
  - `MAX_HISTORY = 50`
  - `HISTORY_PATH = Path.home() / ".whispertype" / "history.json"`
- icon.py — imported by 2 files:
  - `@lru_cache(maxsize=None) def make_tray_icon(state="idle"):`
  - `@lru_cache(maxsize=None) def png_bytes(state="idle"):`
